...
class LLMService(ILLMService):
    ...
    def _roles_instruction(self, config: dict) -> str:
        ...
        if s2:
            lines.append(
                f"- {s1} stellt Fragen und moderiert; {s2} erklärt und liefert Fakten/Beispiele."
            )
    ...
    def _user_prompt(self, thema: str | None, config: dict) -> str:
        ...
        try:
            max_chars = int(
                config.get("source_max_chars", self.DEFAULT_MAX_SOURCE_CHARS)
            )
        except (TypeError, ValueError):
            max_chars = self.DEFAULT_MAX_SOURCE_CHARS
    ...
    def _ask_gemini(self, prompt: str) -> str:
        ...
        Parameter:
        - prompt: Der vollständige Prompt-Text (System + User), der an Gemini gesendet wird

        Rückgabe:
        - String mit dem von Gemini generierten Antworttext

        Exceptions:
        - LLMServiceError: Wenn der Aufruf fehlschlägt oder die Antwort nicht verarbeitet werden kann
        """
        ...
        for attempt in range(self.MAX_ATTEMPTS):
            try:
                ...
                raise LLMServiceError(
                    f"Gemini ist nicht erreichbar (Internet/Server-Problem): {e}"
                )
            ...
            if response.status_code != 200:
                raise LLMServiceError(
                    f"Gemini API-Fehler: HTTP {response.status_code} - {response.text}"
                )
            ...
            try:
                ...
                raise LLMServiceError(
                    f"Gemini-Antwort konnte nicht gelesen werden: {e}"
                )
